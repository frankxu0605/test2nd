        ...
        customRenderTable(items, crud) {
            ...
            tbody.innerHTML = items.map(item => {
                ...
                const overdueBtn = item.overdue_reported
                    ? `<span class="status-badge purple" style="font-size:12px;background:linear-gradient(135deg,#9c27b0,#673ab7);color:#fff;padding:4px 8px;border-radius:12px;box-shadow:0 2px 4px rgba(156,39,176,0.3);display:inline-flex;align-items:center;gap:4px;" title="${item.overdue_reported_at ? '上报时间: ' + item.overdue_reported_at : ''}"><i style="font-size:10px;">✓</i>已上报公共池${item.overdue_reported_at && item.overdue_reported_at !== '已上报' ? '<br><span style="font-size:10px;opacity:0.8;">' + item.overdue_reported_at + '</span>' : ''}</span>`
        ...
        reportOverdue(orderId) {
            ...
            Modal.open('上报逾期公共池', html, async () => {
                const data = {
                    ...
                    reported_by: (() => {
                        console.log('API.tenantInfo:', API.tenantInfo);
                        console.log('API.userInfo:', API.userInfo);
                        return API.tenantInfo?.name || API.userInfo?.real_name || '';
                    })(),
                };
